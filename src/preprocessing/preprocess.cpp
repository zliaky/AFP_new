...
int save(string filename, vector<Data*> &dataset, bool istrain) {
	...
	for (int i = 0; i < dataset.size(); ++i) {
		...
		fout << ad->literal_map << ", " << ad->creative_id_map << ", " << ad->category_id_map << ", " << ad->series_id_map << ", " << ad->advertiser_id_map << ", " << ad->product_type << ", " << ad->product_id_map << ", ";
